...
class TrendingMusic extends StatefulWidget {
  ...
  @override
  _TrendingMusicstate createState() => _TrendingMusicstate();
}
...
class _TrendingMusicstate extends State<TrendingMusic> {
  ...
  Future<void> _loadMoreVideos() async {
    ...
    try {
      List<Video> newVideos = await youtubeDataApi.fetchTrendingMusic();
      setState(() {
...
class YoutubeDataApi {
  ...
  Future<List<Video>> fetchTrendingMusic() async {
    String url = '$baseUrl/videos?part=snippet&chart=mostPopular&maxResults=10&regionCode=US&videoCategoryId=10&key=$apiKey';
